...
export class TeachGroupPP extends Component {
    ...
    onClick = e => {
        ...
        this.setState({ teach_groop: this.props.newId })
    }
        ...
        return (
            <Fragment>
                ...
                <table className="table table-striped table-sm border mt-5">
                    ...
                    <tbody>
                        {this.props.teachgroupstudent.filter(u => u.teach_groop == this.props.newId).map(usr => (
                            <tr key={usr.id}>
                                <td>{this.props.students.filter(sf => sf.id == usr.student).map(us => us.second_name + " " + us.name + " " + us.last_name)}</td>
                                <td><Fragment>
                                    <a
                                        className="text-secondary font-weight-bold"
                                        onClick={() => this.setState({ id: usr.id, student: usr.student, checkOper: "1", showRes: "1" })}
                                    >PUT</a>
                                </Fragment></td>
                                <th><a
                ...
                <div className="d-flex justify-content-center" data-toggle="modal" data-target="#modalLoginFormTG">
...
export default connect(mapStateToProps, { getTeachgroupstudent, addTeachgroupstudent, putTeachgroupstudent, deleteTeachgroupstudent, getStudents })(TeachGroupPP)